...
export default class BookmarksService extends Service {
  ...
  async fetchBookmarks() {
    ...
    if (response.ok) {
      ...
      this.bookmarks = await Promise.all(data.map((bookmark) => this.jsonToBookmark(bookmark)));
    } else {
      const error = await response.text();
      warn(`Failed to fetch bookmarks: [${response.status}] ${error}`, { id: 'bookmarks.failure' });
      this.reset();
    }
  }

  async bookmark(publicServiceId) {
    const response = await fetch(`/public-services/${publicServiceId}/bookmarks`, {
      method: 'POST',
      headers: {
    ...
    if (response.ok) {
      ...
      const error = await response.text();
      warn(`Failed to create bookmark: [${response.status}] ${error}`, { id: 'bookmarks.failure' });
      return null;
    }
  }
